...
for i in accessions:
    ...
    bcount = subprocess.check_output(f'echo $(zcat {p_data_raw}/{i}_1.fastq.gz|wc -l)/4|bc', stdout=subprocess.PIPE, shell=True)
    acount = subprocess.check_output(f'echo $(zcat {p_data_clean}/{i}_mapped_1.fq.gz|wc -l)/4|bc', stdout=subprocess.PIPE, shell=True)
